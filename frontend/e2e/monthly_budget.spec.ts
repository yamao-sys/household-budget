...
test.describe("/monthly_budget", () => {
  test("当月操作", async ({ page }) => {
    ...
    await expect(page.getByText("残高: ¥1,065,000", { exact: true })).toBeVisible();
    ...
    // NOTE: 結果が支出と残高に反映されること
    await expect(page.getByText("収入合計: ¥1,100,000", { exact: true })).toBeVisible();
    await expect(page.getByText("支出合計: ¥38,000", { exact: true })).toBeVisible();
    await expect(page.getByText("残高: ¥1,062,000", { exact: true })).toBeVisible();
    ...
    await expect(page.getByText("残高: ¥1,060,000", { exact: true })).toBeVisible();
    await expect(page.getByText("支出: ¥18000", { exact: true })).toBeVisible();
    ...
    await expect(page.getByText("残高: ¥0", { exact: true })).toBeVisible();
  });
